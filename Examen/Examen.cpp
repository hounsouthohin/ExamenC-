...
int main()
{
	...
	int niveau = Utilitaire::genererNb(2,3);
	Aventurier* aventurier = new Aventurier(niveau);
	int i = 0;
	while (i < 12)
	{
		Quete* quete(0);
		quete = Factory::getRandomQuete();
		if (aventurier->getNiv() >= quete->getDifficulte()) { aventurier->ajouterQuete(quete); i++; }
		i++;
	}
